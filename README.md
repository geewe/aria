...
### 🎤 语音交互与唤醒
- **智能 VAD 唤醒**（零配置，国内网络友好）：FFT 频谱分析 + 自动环境校准，只响应人声忽略噪声
- **Porcupine 唤醒词**（可选）：需 Picovoice 免费 Access Key，精确检测 "Computer" 等关键词
- **唤醒流程**：VAD 检测人声 → 自动开始聆听 → STT 识别 → LLM 处理 → TTS 回复
...
- **波形动画**：聆听时实时音频可视化
- **智能 VAD 唤醒**（零配置）：FFT 频谱分析只响应人声, 自动校准环境
- **Porcupine 唤醒词**（可选）：需 Picovoice 免费 Access Key
- **桌面通知**：唤醒/回复推送系统通知
- **完全离线唤醒**：不依赖任何云服务, 国内网络友好
...
| 唤醒词 | 智能 VAD (FFT+频谱分析, 离线零配置) / Porcupine (可选) |
